...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function isWall(x, y) {
        ...
        return cell === 1 || cell === 4; // ✅ Pacman darf 4 nicht betreten
    }
    ...
    function chooseGhostDirection(g) {
        ...
        // ❌ Rückwärts nach Teleport einmal verbieten
        if (g.blockReverseUntilDecision && g.lastDirBeforeTeleport) {
            ...
            // Failsafe: falls nichts übrig bleibt
            if (!options.length) {
    ...
    function gameLoop() {
        ...
        if (typeof window.onPowerupCollected === "function") {
            const cellValue = PACMAN_GRID[pacman.gridY]?.[pacman.gridX];
            if (cellValue === 2) {
                PACMAN_GRID[pacman.gridY][pacman.gridX] = 0;
                const cell = board.children[pacman.gridY * cols + pacman.gridX];
                const pEl = cell.querySelector(".powerup");
                if (pEl) cell.removeChild(pEl);
        ...
        ghosts.forEach(g => {
            ...
            // ================= Ghost im Haus =================
            if (!g.leftHouse) {
                if (g.atCenter()) { // Nur in Zellmitte Entscheidungen treffen
                    // 1. Alle Felder mit Wert 8 sammeln
                    const exitCells = [];
                    PACMAN_GRID.forEach((row, y) => {
                        row.forEach((v, x) => {
                            if (v === 8) exitCells.push({ x, y });
                        });
                    });

                    if (exitCells.length > 0) {
                        // 2. Kürzesten Pfad zu einem der 8-Felder finden
                        ...
                        // 3. Ersten Schritt auf dem kürzesten Pfad setzen
                ...
                // Haus verlassen, sobald Feld 8 erreicht
                if (PACMAN_GRID[g.gridY]?.[g.gridX] === 8) {
                    g.leftHouse = true;
                }
            }
